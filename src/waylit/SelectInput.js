...
const DropDown = (props) => {
  ...
  return (
    ...
    { state.isOpen ? 
      (
        <ul
          className="absolute z-10 mt-1 w-full bg-white dark:bg-gray-800 shadow-lg max-h-56 rounded-md py-1 text-base ring-1 ring-gray-300 dark:ring-gray-700 overflow-auto focus:outline-none sm:text-sm"
          tabIndex="-1"
          role="listbox"
          ...
          {selectInputData.map(user => (
              <li
                ...
                className={`${user.id === state.selected.id ? 'font-semibold' : 'font-normal'} group cursor-pointer select-none relative py-2 pl-3 pr-9 text-gray-900 dark:text-gray-100 bg-transparent hover:bg-indigo-600 dark:hover:bg-indigo-300`}>
  ...
  return (
    <div>
      ...
      <div className="relative">
        <button
            type="button"
            className="relative w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-md shadow-sm pl-3 pr-10 py-3 text-left cursor-default dark:text-gray-100 focus:input-focus sm:text-sm"
